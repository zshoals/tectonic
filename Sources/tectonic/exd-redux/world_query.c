...
void exd_query_init_from(exd_query_t * q, exd_entset_t * base_set, allocator_t * mem)
{
	exd_entset_init(&q->matcher, mem);
	exd_entset_copy_data_from(&q->matcher, base_set);
}

void exd_query_include(exd_query_t * q, exd_entset_t * component)
{
	exd_entset_and(&q->matcher, component);
}

void exd_query_optional_include(exd_query_t * q, exd_entset_t * component)
{
	exd_entset_or(&q->matcher, component);
}

void exd_query_exclude(exd_query_t * q, exd_entset_t * component)
{
	exd_entset_not(&q->matcher, component);
}
