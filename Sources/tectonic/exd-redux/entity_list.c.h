...
void exd_entity_list_init(exd_entity_list_t * manifest, allocator_t * mem)
{
	...
	exd_hientset_init(&manifest->entity_states, mem);
...
exd_entity_t exd_entity_list_get_new_entity(exd_entity_list_t * manifest)
{
	...
	exd_hientset_set_slot(&manifest->entity_states, id);
...
void exd_entity_list_free_entity(exd_entity_list_t * manifest, exd_entity_t id)
{
	...
	exd_hientset_clear_slot(&manifest->entity_states, id);
...
void exd_entity_list_copy_to_entset(exd_entity_list_t * manifest, exd_hientset_t * destination)
{
	exd_hientset_copy_data_from(destination, &manifest->entity_states);
}

bool exd_entity_list_is_entity_valid(exd_entity_list_t * manifest, exd_entity_t id)
{
	bool is_alive = exd_hientset_entity_exists(&manifest->entity_states, id); 
